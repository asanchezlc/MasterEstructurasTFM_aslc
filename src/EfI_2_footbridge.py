...
sapfile_name = 'Pasarela_v8.sdb'  # 'frame.sdb'
# considered_modes = [1, 2, 7, 8, 9, 18, 25, 26, 27]  # either None (for all) or list (faster execution)
considered_modes = [1, 4, 15, 27, 28, 38]  # either None (for all) or list (faster execution)
...
modal_forces = sap2000.get_modalforces(Name_elements_group, SapModel, average_values=average_values,
                                       round_coordinates=round_coordinates, considered_modes=considered_modes)
...
SavePath = os.path.join(paths['project'], 'src',
                        'EfI_data_footbridge', 'modal_results.json')
with open(SavePath, 'w') as json_file:
    json.dump(modal_results, json_file)

# Joint information
SavePath = os.path.join(paths['project'], 'src',
                        'EfI_data_footbridge', 'joint_coordinates.json')
with open(SavePath, 'w') as json_file:
    json.dump(joint_coordinates, json_file)

# Element information
SavePath = os.path.join(paths['project'], 'src',
                        'EfI_data_footbridge', 'element_coordinates.json')
with open(SavePath, 'w') as json_file:
...
# 4. Save results
SavePath = os.path.join(paths['project'], 'src',
                        'EfI_data_footbridge', filename_SG + '.json')
with open(SavePath, 'w') as json_file:
...
SavePath = os.path.join(paths['project'], 'src',
                        'EfI_data_footbridge', filename_SG_loc)
...
# 4.Save results
SavePath = os.path.join(paths['project'], 'src',
                        'EfI_data_footbridge', filename_accel + '.json')
with open(SavePath, 'w') as json_file:
...
SavePath = os.path.join(paths['project'], 'src',
                        'EfI_data_footbridge', filename_accel_loc)
...
"""
# A) Ed and FIM plots
savefigures_path = os.path.join(paths['savefigures'], 'EFI_footbridge')
if not os.path.exists(savefigures_path):
    os.makedirs(savefigures_path)

if 'Ed_all_accel' not in globals():
    Ed_all_accel = [0, 0]
    detFIM_accel = [0, 0]
    n_sensors_accel = 0
    modes_considered_accel = [0]
